...
export const CompilationIcon = forwardRef<CompilationIconElement, CompilationIconProps>(({ script }, ref) => {
  return match(script.status)
    .with('idle', () => null)
    .with('running', () => (
      <MotionBoltIcon ref={ref} className="h-6 w-6 text-blue-400 dark:text-blue-500" {...fadeAnimate} />
    ))
    .with('busy', () => <MotionBellSnoozeIcon ref={ref} className="h-6 w-6 text-gray-400" {...fadeAnimate} />)
    .with('done', () => <MotionCheckCircleIcon ref={ref} className="h-6 w-6 text-green-400" {...fadeAnimate} />)
